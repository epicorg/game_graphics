...
public class ObjObject {
    ...
    private ArrayList<SFVertex4f> txCoordinates = new ArrayList<SFVertex4f>();
    ...
    public ArrayList<SFVertex4f> getTxCoordinates() {
    ...
    public ObjObject generateReworked() {
        ...
        for (int i = 0; i < indices.size(); i++) {
            ...
            vt.set(this.txCoordinates.get(list.get(0)));
            for (int j = 1; j < list.size(); j++) {
                v.add(this.vertices.get(list.get(j)));
                vn.add(this.normals.get(list.get(j)));
                vt.add(this.txCoordinates.get(list.get(j)));
            }
            ...
            object.getTxCoordinates().add(vt);
        }
    ...
    public void rewriteNormals() {
        ...
        for (int i = 0; i < indices.size(); i++) {
            ...
            for (int index : indices)
                referenceVertices.get(index).add(N);
        }
